...
// Solution code here...
  let longStr = arr.reduce((acc, cur) => acc.length < cur.length ? acc = cur : acc, '');
  let indxOfLongStr = arr.indexOf(longStr);
  return indxOfLongStr;
};
...
const firstLetters = (arr) => {
  // Solution code here...
 let FirLet = arr.map(val => val.charAt(0));
 return FirLet;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
 return arr.map(val => val.split('').filter(value => /[0-9]/.test(value)).join(''));
...
const onlyOddChars = (str) => {
  // Solution code here...
  let oddChar = str.split('').filter((value, idx) => idx % 2 !== 0)
  let joined = oddChar.join('')
  return joined;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let newBolean = arr.reduce((acc, cur) => cur.includes(':)') ? acc : acc = false, true );
  return newBolean;
};
